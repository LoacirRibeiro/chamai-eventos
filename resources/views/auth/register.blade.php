<x-guest-layout>
    <form method="POST" action="{{ route('register') }}" class="space-y-5">
        ...
        <div>
            ...
            <x-text-input id="name" class="block mt-1 w-full !rounded-md border-slate-200 focus:ring-indigo-500" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" placeholder="Ex: Loacir Santos" />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>

        <div>
            <label class="block text-xs font-black uppercase text-slate-400 mb-2 ml-1">E-mail</label>
            <x-text-input id="email" class="block mt-1 w-full !rounded-md border-slate-200 focus:ring-indigo-500" type="email" name="email" :value="old('email')" required autocomplete="username" placeholder="seu@email.com" />
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>

        <div>
            <label class="block text-xs font-black uppercase text-slate-400 mb-2 ml-1">Senha</label>
            <x-text-input id="password" class="block mt-1 w-full !rounded-md border-slate-200 focus:ring-indigo-500" type="password" name="password" required autocomplete="new-password" />
        ...
        <div class="pt-4">
            <button class="w-full bg-slate-900 text-white py-4 rounded-md font-black text-sm hover:bg-slate-800 transition shadow-lg shadow-slate-100 uppercase tracking-widest">
                Criar Minha Conta
            </button>
        </div>
