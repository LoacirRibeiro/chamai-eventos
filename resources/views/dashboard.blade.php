<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-2">
            ...
            <a href="{{ route('events.create') }}" class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3.5 rounded-xl font-black text-xs transition shadow-lg shadow-indigo-100 flex items-center justify-center gap-2 uppercase tracking-widest">
    ...
    <div class="py-6 md:py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    @foreach(auth()->user()->unreadNotifications as $notification)
                        <div class="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="w-8 h-8 shrink-0 bg-indigo-50 text-indigo-500 rounded-lg flex items-center justify-center">
                            ...
                            <a href="{{ $notification->data['link'] }}" class="ml-4 px-3 py-1.5 text-[9px] font-black bg-slate-50 text-slate-500 rounded-lg hover:bg-indigo-600 hover:text-white transition uppercase">
                                Ver
                            </a>
                        </div>
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <span class="text-[9px] font-black text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-full uppercase">
                {{ $events->count() }} Total
            </span>
        </div>
            ...
            @forelse ($events as $evento)
                <div class="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col">
                    <div class="p-6 flex-1">
                        ...
                        {{-- DATA DO EVENTO (Ajustado para usar data_horario) --}}
                        <div class="flex justify-between items-start mb-6">
                            <div class="bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 text-center transition group-hover:bg-indigo-600 group-hover:border-indigo-600">
                            ...
                            <div class="text-right">
                                <span class="text-[9px] font-black bg-emerald-50 text-emerald-600 px-2 py-1 rounded-lg uppercase border border-emerald-100">
                        ...
                        <div class="bg-slate-50/50 rounded-xl p-3 border border-slate-50 text-center">
                            @php
                    ...
                    <div class="px-6 py-4 bg-slate-50/30 border-t border-slate-50 flex items-center justify-between">
                         ...
                         <a href="{{ route('events.show', $evento->id) }}" class="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg font-black text-[10px] uppercase hover:bg-indigo-600 transition shadow-sm">
                            Gerenciar <i class="fa-solid fa-chevron-right text-[8px]"></i>
                         </a>
                    </div>
                </div>
            @empty
                <div class="col-span-full text-center py-20 bg-white rounded-2xl border-2 border-dashed border-slate-100">
                    ...
                    <a href="{{ route('events.create') }}" class="mt-4 bg-indigo-600 text-white px-8 py-4 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-indigo-700 transition inline-flex items-center gap-3">
