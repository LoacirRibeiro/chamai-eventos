    ...
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4 px-1">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <h2 class="font-black text-base md:text-2xl text-slate-800 tracking-tight truncate leading-tight">Itens da Festa</h2>
                    <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest truncate">
                         <i class="fa-solid fa-bolt-lightning text-indigo-400 mr-1"></i> {{ $event->titulo }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 space-y-4 md:space-y-8">
            ...
            <div class="bg-white p-4 rounded-xl border border-slate-100 shadow-sm md:max-w-sm">
                ...
                <form action="{{ route('itens.store', $event) }}" method="POST" class="flex flex-col gap-3">
                    ...
                    <input type="text" name="nome" placeholder="Nome do Item" required 
                           class="w-full border-slate-100 bg-slate-50 rounded-lg font-bold text-xs focus:ring-indigo-500 py-2.5">
                    <input type="number" name="quantidade" placeholder="Qtd. Total" min="1" required 
                           class="w-full border-slate-100 bg-slate-50 rounded-lg font-bold text-xs focus:ring-indigo-500 py-2.5">
                    <button type="submit" class="w-full bg-indigo-600 text-white py-3 rounded-lg font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition">
                        Adicionar à Lista
                    </button>
                </form>
            </div>
            ...
            <div class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                <div class="p-4 border-b border-slate-50 bg-white">
                    <h3 class="font-black text-[10px] md:text-xs text-slate-800 uppercase tracking-widest">Itens Necessários</h3>
                </div>

                {{-- O Pulo do Gato: Double Wrapper para Rolagem Perfeita --}}
                <div class="w-full overflow-x-auto">
                    <div class="inline-block min-w-full align-middle">
                        <table class="min-w-full divide-y divide-slate-100">
                            <thead>
                                <tr class="bg-slate-50/50">
                                    <th class="px-4 py-3 text-[9px] font-black text-slate-400 uppercase text-left">Item</th>
                                    <th class="px-4 py-3 text-[9px] font-black text-slate-400 uppercase text-center">Qtd</th>
                                    <th class="px-4 py-3 text-[9px] font-black text-slate-400 uppercase text-center">Vagas</th>
                                    <th class="px-4 py-3 text-[9px] font-black text-slate-400 uppercase text-right">Ações</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-50 bg-white">
                                @foreach($event->itens as $item)
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition">
                                        <td class="px-4 py-4 whitespace-nowrap font-bold text-[11px] text-slate-700">{{ $item->nome }}</td>
                                        <td class="px-4 py-4 whitespace-nowrap text-center font-bold text-[11px] text-slate-500">{{ $item->quantidade }}</td>
                                        <td class="px-4 py-4 whitespace-nowrap text-center">
                                            <span class="px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter {{ $vagasLivres > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
            ...
            <div class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                <div class="p-4 border-b border-slate-50 bg-white">
                    <h3 class="font-black text-[10px] md:text-xs text-slate-800 uppercase tracking-widest">Quem leva o quê?</h3>
                </div>
                <div class="w-full overflow-x-auto">
                    <div class="inline-block min-w-full align-middle">
                        <table class="min-w-full divide-y divide-slate-100">
                            ...
                            <tbody class="divide-y divide-slate-50 bg-white">
                                @foreach($event->convidados as $convidado)
                                    <tr class="hover:bg-slate-50/50 transition">
                                        <td class="px-4 py-4 whitespace-nowrap font-bold text-[11px] text-slate-700">{{ $convidado->nome }}</td>
                                        <td class="px-4 py-4 text-center">
                                            @forelse($convidado->itens as $itemViculado)
                                                <span class="inline-block bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded text-[8px] font-black uppercase mb-1">
                                                    {{ $itemViculado->pivot->quantidade_levada }}x {{ $itemViculado->nome }}
                                                </span>
                                            @empty
                                                <span class="text-slate-300 text-[9px] font-bold italic">Nenhum item</span>
                                            @endforelse
                                        </td>
                                        <td class="px-4 py-4 text-right">
                                            <form action="#" method="POST" class="inline-block">
                                                ...
                                                <select onchange="atribuirItem(this, '{{ $convidado->id }}')" 
                                                        class="text-[9px] font-black uppercase bg-slate-50 border-slate-100 rounded-lg focus:ring-0 cursor-pointer py-1 px-2">
